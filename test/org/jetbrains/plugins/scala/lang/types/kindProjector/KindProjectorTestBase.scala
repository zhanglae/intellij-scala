...
abstract class KindProjectorTestBase extends ScalaLightPlatformCodeInsightTestCaseAdapter {
  ...
  protected def doTest(code: String): Unit = {
    val trimmed = code.trim
    configureFromFileTextAdapter(getTestName(false) + ".scala", trimmed)
    val scalaFile = getFileAdapter.asInstanceOf[ScalaFile]
    val offset = trimmed.indexOf(startExprMarker)
    ...
    val endOffset = trimmed.indexOf(endExprMarker)
